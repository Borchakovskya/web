...
$db = new PDO('mysql:host=localhost;dbname=u16852', $user, $pass, array(PDO::ATTR_PERSISTENT => true));
...
// =========== POST ===========
  ...
  if (!empty($_COOKIE[session_name()]) && session_start() && !empty($_SESSION['login'])) {
    ...
    try {
      ...
      $stmt = $db->prepare("UPDATE web5 SET name = ?, email = ?, year = ?, gender = ?, limbs = ?, powers = ?, bio = ? WHERE login = ?");
      $stmt->execute(array(
        $_POST['name'],
        $_POST['email'],
        $_POST['year'],
    ...
    try {
      $stmt = $db->prepare("INSERT INTO web5 SET login = ?, pass = ?, name = ?, email = ?, year = ?, gender = ?, limbs = ?, powers = ?, bio = ?");
      $stmt->execute(array(
        $user_login,
